...
struct npc_rinling_darkmoon : public ScriptedAI
{
    npc_rinling_darkmoon(Creature* creature) : ScriptedAI(creature)
    {
        Reset();
    }
...
// spell - 101872
class spell_heshoots_shoot_hit : public SpellScript
{
private:
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_heshoots_shoot_hit::SetTargets, EFFECT_0, TARGET_UNIT_CONE_ENTRY);
        OnEffectHitTarget += SpellEffectFn(spell_heshoots_shoot_hit::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};

// spell - 101010
class spell_heshoots_indicator : public AuraScript
{
    ...
    void Register()
    {
        OnEffectApply += AuraEffectApplyFn(spell_heshoots_indicator::OnApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectApplyFn(spell_heshoots_indicator::EffectRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};

void AddSC_darkmoon_shooting_gallery()
{
    RegisterCreatureAI(npc_rinling_darkmoon);
    RegisterSpellScript(spell_heshoots_shoot_hit);
    RegisterSpellScript(spell_heshoots_indicator);
};
